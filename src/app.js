...
(function () {
    ...
    function applyGradedView(data, selectedIndex) {
        try { haptic(data.correct ? 'success' : 'error'); } catch (e) {}
    ...
    function openPrintExam(qs, cat) {
        ...
        const html = `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>body{font-family:Georgia,'Times New Roman',serif;color:#111;max-width:740px;margin:0 auto;padding:32px 24px;line-height:1.5;}h1{font-size:20px;margin:0 0 4px;}.sub{color:#555;font-size:13px;margin:0 0 24px;}.q{margin:0 0 20px;page-break-inside:avoid;}.qn{font-weight:bold;}.cat{color:#777;font-weight:normal;font-size:12px;}.stem{margin:4px 0 8px;}.opt{margin:2px 0 2px 18px;}.key{page-break-before:always;}.key h2{font-size:18px;border-bottom:2px solid #111;padding-bottom:4px;}.ans{margin:0 0 12px;font-size:13px;}.src{color:#777;font-size:11px;margin-top:2px;}</style></head><body><h1>${esc(title)}</h1><p class="sub">Generated by MACPrep · macprep.org · Answer key follows the questions.</p>${qHtml}<div class="key"><h2>Answer key &amp; explanations</h2>${keyHtml}</div></body></html>`;
        printHtmlDoc(html);
    }
    // Print an HTML document via a hidden same-document iframe → invokes the native print /
    // Save-to-Files sheet. Works in the iOS WebView, where window.open() returns null.
    function printHtmlDoc(html) {
        try {
            ...
            setTimeout(() => { try { ifr.contentWindow.focus(); ifr.contentWindow.print(); } catch (e) {} setTimeout(() => { try { ifr.remove(); } catch (e) {} }, 60000); }, 250);
        } catch (e) { toast('Could not open the print view.'); }
    }
...
`;
    ...
    // browser for off-domain links, Android hardware back. All feature-detected → inert on web.
    ...
    // Match the status-bar glyphs to the active theme (dark glyphs on light themes, light on dark).
    function syncNativeStatusBar() {
        const SB = capPlugin('StatusBar'); if (!SB) return;
        try {
            const bg = getComputedStyle(document.body).backgroundColor;
            const m = (bg || '').match(/\d+/g);
            let darkBg = false;
            if (m) { const lum = 0.299 * +m[0] + 0.587 * +m[1] + 0.114 * +m[2]; darkBg = lum < 128; }
            SB.setStyle({ style: darkBg ? 'LIGHT' : 'DARK' }); // Style.Light = light glyphs (for a dark bg)
            if (window.Capacitor.getPlatform && window.Capacitor.getPlatform() === 'android') {
                try { SB.setBackgroundColor({ color: rgbToHex(bg) }); } catch (e) {}
            }
        } catch (e) {}
    }
    // A light tap you can feel — gated to native, no-ops on the web.
    ...
    function initNativeShell() {
        ...
        if (Browser) {
            document.addEventListener('click', (e) => {
                ...
                if (/^(mailto:|tel:|sms:|#)/i.test(href)) return; // system handles these; in-page anchors stay
                let host = ''; try { host = new URL(a.href, location.href).host; } catch (_) { return; }
                if (host && host !== location.host) {
                    e.preventDefault();
                    Browser.open({ url: a.href, presentationStyle: 'popover' }).catch(() => {});
                }
            }, true);
        }
        // Android hardware back → close a modal, else go to the dashboard (never a silent mid-quiz exit).
