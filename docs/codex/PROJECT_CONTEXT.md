...
- The first AA-lifecycle expansion is implemented: applicants receive free sourced information and private planning resources; an applicant who has accepted an offer can record the committed program, matriculation date, and expected graduation date during signup or later; incoming students transition into the SAA experience on their matriculation date; and student accounts transition into the practicing CAA experience on their stored graduation date so professional and CME resources can be timed appropriately. Applicants without a commitment receive at most one in-app acceptance check-in every 30 days and can pause it until a later cycle. This is a product lifecycle label, not independent verification of certification or licensure. `lifecycle_stage` remains separate from premium entitlement so applicants and incoming students never enter SAA benchmarks or cohort analytics, and the SAA transition opens only the existing 25-question free trial unless the account already has premium.
- Admissions preparation is a free relationship-building surface, not a paid coaching product. The applicant workspace is an information hub: date every volatile fact, link to the primary program/CAAHEP/AAAA/NCCAA/CASAA source, and label unreported or unverified school statistics instead of estimating them. MACPrep should not score an applicant's competitiveness or claim admissions expertise it does not have. Aspiring CAA is the named independent resource for Sarah Whitfield's admissions guidance and should remain linked from the applicant workspace and public CAA-resource footers.
